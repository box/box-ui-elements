 ...
 */
function useSharedLink(api, itemID, itemType, permissions, accessLevel, options = {}) {
  ...
  const [generatedFunctions, setGeneratedFunctions] = React.useState(false);
  ...
  React.useEffect(() => {
     ...
     */
    const updatedOnAddLinkFn = () => () => connectToItemShare({
      successFn: data => {
        const {
          shared_link: {
            access
    ...
    const updatedOnRemoveLinkFn = () => () => connectToItemShare({
      access: ACCESS_NONE,
    ...
    const updatedChangeSharedLinkAccessLevelFn = () => newAccessLevel => connectToItemShare({
      // $FlowFixMe
    ...
    const connectToUpdateSharedLink = newSharedLinkData => {
      setIsLoading(true);
    ...
    const updatedChangeSharedLinkPermissionLevelFn = () => newSharedLinkPermissionLevel => connectToUpdateSharedLink({
     ...
     */
    const updatedOnSubmitSettingsFn = () => newSettings => connectToUpdateSharedLink(transformSettings(newSettings, currentAccessLevel.current));
