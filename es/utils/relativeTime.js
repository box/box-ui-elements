...
const timeFromNow = ms => {
  ...
  if (Math.abs(diff) >= YEAR_IN_MS) {
    return {
      value: Math.trunc(diff / YEAR_IN_MS),
      unit: 'year'
    };
  }
  if (Math.abs(diff) >= WEEK_IN_MS) {
    return {
      value: Math.trunc(diff / WEEK_IN_MS),
      unit: 'week'
    };
  }
  if (Math.abs(diff) >= DAY_IN_MS) {
    return {
      value: Math.trunc(diff / DAY_IN_MS),
      unit: 'day'
    };
  }
  if (Math.abs(diff) >= HOUR_IN_MS) {
    return {
      value: Math.trunc(diff / HOUR_IN_MS),
      unit: 'hour'
    };
  }
  if (Math.abs(diff) >= MIN_IN_MS) {
    return {
      value: Math.trunc(diff / MIN_IN_MS),
      unit: 'minute'
    };
  }
  return {
    value: Math.trunc(diff / SEC_IN_MS),
    unit: 'second'
  };
};
