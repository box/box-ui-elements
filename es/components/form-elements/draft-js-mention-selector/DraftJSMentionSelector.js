...
class DraftJSMentionSelector extends React.Component {
  constructor(props) {
    super(props);
    _defineProperty(this, "getIsVideoTimestampEnabled", () => {
      const {
        timestampLabel
      } = this.props;
      return !!timestampLabel && timestampLabel.trim() !== '';
    });
    _defineProperty(this, "toggleTimestamp", (editorState, forceOn = false) => {
      if (!editorState) return;
      const currentContent = editorState.getCurrentContent();
      let updatedContent;
      let newIsTimestampToggledOn;
      const {
        isTimestampToggledOn
      } = this.state;
      ...
      if ((!isTimestampToggledOn || forceOn) && !isTimestampEntityPresent) {
        ...
        // $FlowFixMe
        const contentWithTimestampEntity = currentContent.createEntity(UNEDITABLE_TIMESTAMP_TEXT,
        // Entity type
        'IMMUTABLE', {
          timestampInMilliseconds,
          fileVersionId
        ...
        const selectionAtStart = SelectionState.createEmpty(contentWithTimestampEntity.getFirstBlock().getKey()).merge({
          anchorOffset: 0,
          focusOffset: 0
        });
    ...
    _defineProperty(this, "getIsTimestampEntityPresent", currentContent => {
     ...
     */
    _defineProperty(this, "getTimestampLength", currentContent => {
      // $FlowFixMe
    ...
    _defineProperty(this, "getVideoTimestamp", () => {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    const {
      ...
      isRequired: prevIsRequiredFromProps
    } = prevProps;
    const {
      editorState,
      isRequired
  ...
  render() {
    ...
    const {
      ...
      internalEditorState,
      error,
      isTimestampToggledOn: timestampToggledOn
    } = this.state;
    const {
      handleBlur,
      handleChange,
    ...
    let editorState = internalEditorState || externalEditorState;
    ...
    }), isRequired && this.getIsVideoTimestampEnabled() && /*#__PURE__*/React.createElement(Toggle, {
