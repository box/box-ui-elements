import * as React from 'react';
import IconVerified from '../../icons/general/IconVerified';
import IconTaskGeneral from '../../icons/two-toned/IconTaskGeneral';
import IconAnnotation from '../../icons/two-toned/IconAnnotation';

import Avatar from './Avatar';
import notes from './Avatar.stories.md';

export const regular = () => <Avatar id={1} name="Aaron Levie" />;

export const small = () => <Avatar id={1} name="Aaron Levie" size="small" />;

export const large = () => <Avatar id={1} name="Aaron Levie" size="large" />;

export const withAvatarUrl = () => (
    <Avatar
        avatarUrl="data:image/jpeg;base64,"
        id={1}
        name="Aaron Levie"
    />
);

export const withUrlFallbackToInitials = () => <Avatar avatarUrl="garbage" id={1} name="Aaron Levie" />;

export const markedAsExternal = () => (
    <div>
        <Avatar id={1} isExternal name="Aaron Levie" shouldShowExternal size="large" />
        <br />
        <Avatar
            avatarUrl="data:image/jpeg;base64,"
            id={1}
            isExternal
            name="Aaron Levie"
            shouldShowExternal
        />
        <br />
        <Avatar id={1} isExternal name="Aaron Levie" shouldShowExternal size="small" />
    </div>
);

export const withBadges = () => (
    <div>
        <Avatar badgeIcon={<IconTaskGeneral />} id={1} name="Aaron Levie" size="large" />
        <br />
        <Avatar badgeIcon={<IconTaskGeneral />} id={1} name="Aaron Levie" />
        <br />
        <Avatar badgeIcon={<IconTaskGeneral />} id={1} name="Aaron Levie" size="small" />
        <br />
        <br />

        <Avatar badgeIcon={<IconAnnotation />} id={1} name="Aaron Levie" size="large" />
        <br />
        <Avatar badgeIcon={<IconAnnotation />} id={1} name="Aaron Levie" />
        <br />
        <Avatar badgeIcon={<IconAnnotation />} id={1} name="Aaron Levie" size="small" />
        <br />
        <br />

        <Avatar
            badgeIcon={<IconVerified className="completed" title="Verified user" />}
            id={1}
            name="Aaron Levie"
            size="large"
        />
        <br />
        <Avatar
            avatarUrl="data:image/jpeg;base64,"
            badgeIcon={<IconVerified className="completed" title="Verified user" />}
            id={1}
            name="Aaron Levie"
        />
        <br />
        <Avatar
            badgeIcon={<IconVerified className="completed" title="Verified user" />}
            id={1}
            name="Aaron Levie"
            size="small"
        />
    </div>
);

export const withMultipleAvatars = () => (
    <div>
        <Avatar id={1} name="Aaron Levie" />
        <Avatar id={2} name="Front End (example.com)" />
        <Avatar id={3} name="Redwood City" />
    </div>
);

export const withoutNameOrInitials = () => <Avatar />;

export default {
    title: 'Components|Avatar',
    component: Avatar,
    parameters: {
        notes,
    },
};
