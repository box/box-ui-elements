import * as React from 'react';
import IconVerified from '../../icons/general/IconVerified';
import IconTaskGeneral from '../../icons/two-toned/IconTaskGeneral';
import IconAnnotation from '../../icons/two-toned/IconAnnotation';

import Avatar from './Avatar';
import notes from './Avatar.stories.md';

export const regular = () => <Avatar id={1} name="Aaron Levie" />;

export const small = () => <Avatar id={1} name="Aaron Levie" size="small" />;

export const large = () => <Avatar id={1} name="Aaron Levie" size="large" />;

export const withAvatarUrl = () => (
    <Avatar
        id={1}
        name="Aaron Levie"
        avatarUrl="data:image/jpeg;base64,"
    />
);

export const withUrlFallbackToInitials = () => <Avatar id={1} name="Aaron Levie" avatarUrl="garbage" />;

export const markedAsExternal = () => (
    <div>
        <Avatar id={1} shouldShowExternal name="Aaron Levie" isExternal size="large" />
        <br />
        <Avatar
            id={1}
            name="Aaron Levie"
            shouldShowExternal
            isExternal
            avatarUrl="data:image/jpeg;base64,"
        />
        <br />
        <Avatar id={1} shouldShowExternal name="Aaron Levie" isExternal size="small" />
    </div>
);

export const withBadges = () => (
    <div>
        <Avatar id={1} name="Aaron Levie" badgeIcon={<IconTaskGeneral width={20} height={20} />} size="large" />
        <br />
        <Avatar id={1} name="Aaron Levie" badgeIcon={<IconTaskGeneral width={20} height={20} />} />
        <br />
        <Avatar id={1} name="Aaron Levie" badgeIcon={<IconTaskGeneral width={20} height={20} />} size="small" />
        <br />
        <br />

        <Avatar id={1} name="Aaron Levie" badgeIcon={<IconAnnotation width={20} height={20} />} size="large" />
        <br />
        <Avatar id={1} name="Aaron Levie" badgeIcon={<IconAnnotation width={20} height={20} />} />
        <br />
        <Avatar id={1} name="Aaron Levie" badgeIcon={<IconAnnotation width={20} height={20} />} size="small" />
        <br />
        <br />

        <Avatar
            badgeIcon={<IconVerified className="completed" height={12} width={12} title="Verified user" />}
            id={1}
            name="Aaron Levie"
            size="large"
        />
        <br />
        <Avatar
            avatarUrl="data:image/jpeg;base64,"
            badgeIcon={<IconVerified className="completed" height={12} width={12} title="Verified user" />}
            id={1}
            name="Aaron Levie"
        />
        <br />
        <Avatar
            badgeIcon={<IconVerified className="completed" height={12} width={12} title="Verified user" />}
            id={1}
            name="Aaron Levie"
            size="small"
        />
    </div>
);

export const withMultipleAvatars = () => (
    <div>
        <Avatar id={1} name="Aaron Levie" />
        <Avatar id={2} name="Front End (example.com)" />
        <Avatar id={3} name="Redwood City" />
    </div>
);

export const withoutNameOrInitials = () => <Avatar />;

export default {
    title: 'Components|Avatar',
    component: Avatar,
    parameters: {
        notes,
    },
};
