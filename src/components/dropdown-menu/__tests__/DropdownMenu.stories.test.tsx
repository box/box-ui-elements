describe('components/dropdown-menu/DropdownMenu', () => {
    ...
    test.each(DROPDOWN_MENU_STORIES)('looks visually correct when using story %s', async (id: string) => {
        const image = await BoxVisualTestUtils.takeScreenshot(id);
        return expect(image).toMatchImageSnapshot();
    });

    test.each(DROPDOWN_MENU_STORIES)(
        'looks visually correct when button is clicked in story %s',
        async (id: string) => {
