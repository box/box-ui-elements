...
import DatalistItem, { type DatalistItemProps } from '../datalist-item/DatalistItem';
...
const toggleOption = (options: Array<SelectOptionValueProp>, value: SelectOptionValueProp) => {
...
export interface BaseSelectFieldProps {
    ...
    optionRenderer: (option: SelectOptionProp) => React.ReactNode;
...
interface BaseSelectFieldState {
    /** ID of the currently active option for aria-activedescendant */
    activeItemID: string | null;
    /** Index of the currently active option, or -1 when none is active */
    activeItemIndex: number;
    /** Whether the dropdown is open */
    isOpen: boolean;
    /** Current search field text */
    searchText: string;
    /** Whether the active option should be scrolled into view */
...
class BaseSelectField extends React.Component<BaseSelectFieldProps, BaseSelectFieldState> {
    static defaultProps = {
        buttonProps: {},
        isDisabled: false,
    ...
    constructor(props: BaseSelectFieldProps) {
        super(props);
    ...
    selectFieldContainerRef: React.RefObject<HTMLDivElement>;
    ...
    handleBlur = (event?: React.FocusEvent) => {
        ...
        if (
            ...
            exceptionClasses.every(
                className => event && !(event.relatedTarget as HTMLElement).classList.contains(className),
            )
        ) {
            this.closeDropdown();
        }
    ...
    selectMultiOption = (index: number) => {
        ...
        const newSelectedValues = toggleOption(selectedValues, item.value);
    ...
    renderSearchInput = () => {
        ...
        const { searchText } = this.state;
        const getSearchInput = (element: HTMLInputElement | null) => {
    ...
    renderSelectOptions = () => {
        ...
        const selectOptions = filteredOptions.map((item, index) => {
            ...
            const itemProps: Omit<DatalistItemProps, 'children'> & {
                key: number;
...
type InjectedBaseSelectFieldProps = Omit<
    BaseSelectFieldProps,
    'intl' | 'isRightAligned' | 'multiple' | 'optionRenderer' | 'options' | 'selectedValues' | 'separatorIndices'
> &
    Partial<
        Pick<
            BaseSelectFieldProps,
            'isRightAligned' | 'multiple' | 'optionRenderer' | 'options' | 'selectedValues' | 'separatorIndices'
        >
    >;

export { BaseSelectField as BaseSelectFieldBase };
export default injectIntl(BaseSelectField) as React.ComponentType<InjectedBaseSelectFieldProps>;
