...
function makeSelectable<P extends object>(BaseTable: React.ComponentType<P>) {
    ...
    return class SelectableTable extends React.Component<P & MakeSelectableProps, MakeSelectableState> {
        ...
        hotkeys: HotkeyConfig[] | null = null;
        ...
        handleKeyboardSearch = (event: KeyboardEvent) => {
            const { searchStrings } = this.props;
            const target = event.target as HTMLElement | null;

            if (!searchStrings || !target) {
                return;
            }
        ...
        isTargetSlider = (event: KeyboardEvent) => (event.target as HTMLElement | null)?.role === 'slider';

        // Workaround for focus conflicting with Blueprint components for QuickSearch result, recent items and Quick Filters
        isTargetQuickSearch = (event: KeyboardEvent) => {
            const target = event.target as HTMLElement | null;
        ...
        shouldNotAllowArrowKeyNavigation = (event: KeyboardEvent) =>
