...
export interface MakeSelectableProps {
    ...
    /** Number of columns when rendering in grid view */
    gridColumnCount?: number;
    /** Whether the table is displayed as a grid */
    isGridView?: boolean;
    ...
    onSelect: (selectedItems: Array<string | number> | Set<string | number>) => void;
    /**
     ...
     */
    selectedItems?: Array<string | number> | Set<string | number>;
    ...
    loadedData?: Array<string | number>;
...
interface MakeSelectableState {
    /** Index of the currently focused row, or undefined when no row is focused */
    focusedIndex: number | undefined;
}

function makeSelectable<P extends object>(BaseTable: React.ComponentType<P>) {
    const originalDisplayName = BaseTable.displayName || BaseTable.name || 'Table';

    return class SelectableTable extends React.Component<P & MakeSelectableProps, MakeSelectableState> {
        static displayName = `Selectable(${originalDisplayName})`;

        static defaultProps: Partial<MakeSelectableProps> = {
            selectedItems: Set(),
        };
        ...
        blurTimerID: ReturnType<typeof setTimeout> | null;

        constructor(props: P & MakeSelectableProps) {
            super(props);
        ...
        componentDidMount() {
            document.addEventListener('keypress', this.handleKeyboardSearch as unknown as EventListener);
        }

        componentDidUpdate(prevProps: P & MakeSelectableProps, prevState: MakeSelectableState) {
        ...
        componentWillUnmount() {
            document.removeEventListener('keypress', this.handleKeyboardSearch as unknown as EventListener);
            clearTimeout(this.blurTimerID);
        }

        onSelect = (selectedItems: Set<string | number>, newFocusedIndex: number | undefined) => {
            ...
            return [
                ...
                new HotkeyRecord({
                    ...
                    handler: event => {
                        ...
                        this.onSelect(Set(data), this.state.focusedIndex);
                    },
                    type: hotkeyType,
                }),
                new HotkeyRecord({
                    ...
                    handler: () => {
                        this.onSelect(Set(), this.state.focusedIndex);
                    },
                    type: hotkeyType,
                }),
            ];
            ...
            return [
                ...
                new HotkeyRecord({
                    ...
                    handler: () => {
                        ...
                        const newFocusedIndex = Math.min(focusedIndex + 1, data.length - 1);
                        this.handleShiftKeyDownForGrid(newFocusedIndex);
                    },
                    type: hotkeyType,
                }),
                new HotkeyRecord({
        ...
        getProcessedProps = (): P &
            ...
            return {
                ...
                selectedItems: (Set.isSet(selectedItems) ? selectedItems : Set(selectedItems)) as Set<string | number>,
            };
        };

        hotkeys = null;

        selectToggle = (rowIndex: number) => {
        ...
        selectRange = (rowIndex: number) => {
            ...
            const selectedRows = Set(
                data.reduce<number[]>((rows, item, i) => {
        ...
        selectOne = (rowIndex: number) => {
            ...
            this.onSelect(Set([data[rowIndex]]), rowIndex);
            this.anchorIndex = rowIndex;
        };
        ...
        handleRowClick = (event: { metaKey?: boolean; ctrlKey?: boolean; shiftKey?: boolean }, index: number) => {
        ...
        handleRowFocus = (event: unknown, index: number) => {
        ...
        handleShiftKeyDown = (newFocusedIndex: number, boundary: number) => {
            const { data, selectedItems } = this.getProcessedProps();
            const { focusedIndex } = this.state;
        ...
        isContiguousSelection = (selectedItemIndecies: Set<number>, sourceIndex: number, targetIndex: number) => {
        ...
        handleShiftKeyDownForGrid = (newFocusedIndex: number) => {
        ...
        handleCheckboxClick = (event: { nativeEvent: { shiftKey?: boolean } }, index: number) => {
        ...
        isTargetSlider = (event: { target?: { role?: string } }) => event.target?.role === 'slider';

        // Workaround for focus conflicting with Blueprint components for QuickSearch result, recent items and Quick Filters
        isTargetQuickSearch = (event: { target?: { className?: string; dataset?: DOMStringMap } }) => {
            if (!event.target) {
                return false;
            }
        ...
        shouldNotAllowArrowKeyNavigation = (event: { target?: { className?: string; dataset?: DOMStringMap } }) =>
        ...
        render() {
            ...
            const TableComponent = BaseTable as React.ComponentType<
                P & {
                    className?: string;
                    focusedIndex?: number;
                    focusedItem?: string | number;
                    onCheckboxClick: (event: { nativeEvent: { shiftKey?: boolean } }, index: number) => void;
                    onRowClick: (
                        event: { metaKey?: boolean; ctrlKey?: boolean; shiftKey?: boolean },
                        index: number,
                    ) => void;
                    onRowFocus: (event: unknown, index: number) => void;
                    onTableBlur: () => void;
                    onTableFocus: () => void;
                }
            >;
