...
describe('components/pill-selector-dropdown/PillSelectorDropdownField', () => {
    ...
    describe('handleInput()', () => {
        ...
        test('should forward paste input events', () => {
            const onInput = jest.fn();
            const pasteEvent = {
                clipboardData: { getData: jest.fn(() => 'pasted value') },
                target: { value: 'pasted value' },
            };
            const wrapper = getWrapper({ onInput });

            wrapper.prop('onInput')('pasted value', pasteEvent);

            expect(onInput).toHaveBeenCalledWith('pasted value', pasteEvent);
        });

        test('should forward selection-reset events and blur empty input', () => {
            const onBlur = jest.fn();
            const onInput = jest.fn();
            const resetEvent = { target: { value: '' } };
            const wrapper = getWrapper({
                field: { name: 'pill', onBlur },
                onInput,
            });

            wrapper.prop('onInput')('', resetEvent);

            expect(onInput).toHaveBeenCalledWith('', resetEvent);
            expect(onBlur).toHaveBeenCalledWith(resetEvent);
        });
    });
