...
import type { ContactType } from '../../features/unified-share-modal/types';
import type {
    GetPillImageUrl,
    Option,
    OptionValue,
...
class PillSelectorDropdown extends React.Component<PillSelectorDropdownProps, PillSelectorDropdownState> {
    ...
    addPillsFromInput = (inputValue: string) => {
        ...
        if (pills.length > 0) {
            ...
            if (validateForError && (inputValue !== '' || (selectedOptions as Array<Option>).length === 0)) {
                /**
    ...
    handleInput = (
        ...
        const { value } = event.target as HTMLInputElement | HTMLTextAreaElement | { value: string };
    ...
    handleSelect = (index: number, event: React.SyntheticEvent) => {
        ...
        onSelect([selectedOption as Option], event);
        onPillCreate([selectedOption as Option]);
