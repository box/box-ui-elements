...
import { CompositeDecorator, ContentBlock, EditorState, Modifier, SelectionState, ContentState } from 'draft-js';
...
export interface DraftJSMentionSelectorProps {
    ...
    /** External DraftJS editor state when used as a controlled component */
    editorState?: EditorState;
    /** File version id attached to timestamp entities */
    ...
    /** Minimum required comment length */
    minLength?: number;
    /** Form input name */
    name: string;
    ...
    /** Label for the video timestamp toggle; empty or null disables timestamps */
    timestampLabel?: string | null;
    /** Whether validity is checked on blur */
    validateOnBlur?: boolean;
}

interface DraftJSMentionSelectorState {
    /** Contact list synced from props for mention suggestions */
    contacts: SelectorItems;
    /** Current validation error, if any */
    error: object | null | undefined;
    /** Internally owned editor state when the component is uncontrolled */
    internalEditorState: EditorState | null | undefined;
    /** Whether the user has interacted with the editor */
    isTouched: boolean;
    /** Whether a video timestamp entity is currently prepended */
    isTimestampToggledOn: boolean;
}

class DraftJSMentionSelector extends React.Component<DraftJSMentionSelectorProps, DraftJSMentionSelectorState> {
    ...
    constructor(props: DraftJSMentionSelectorProps) {
        super(props);
    ...
    ): Pick<DraftJSMentionSelectorState, 'contacts'> | null {
    ...
    componentDidUpdate(prevProps: DraftJSMentionSelectorProps, prevState: DraftJSMentionSelectorState): void {
        ...
        if (prevEditorState && currentEditorState && prevEditorState !== currentEditorState) {
            ...
            if (newState) {
                this.setState(
                    newState as Pick<DraftJSMentionSelectorState, 'error' | 'isTouched'>,
    ...
    getIsVideoTimestampEnabled = (): boolean => {
        const { timestampLabel } = this.props;
        return !!timestampLabel && timestampLabel.trim() !== '';
    };

    getDerivedStateFromEditorState(
        currentEditorState: EditorState,
        previousEditorState: EditorState,
    ): Partial<Pick<DraftJSMentionSelectorState, 'error' | 'isTouched'>> | null {
        ...
        let newState: Partial<Pick<DraftJSMentionSelectorState, 'error' | 'isTouched'>> | null = null;
    ...
    toggleTimestamp = (editorState?: EditorState | null, forceOn: boolean = false): void => {
    ...
    checkValidityIfAllowed(): void {
        const { validateOnBlur }: DraftJSMentionSelectorProps = this.props;
    ...
    getErrorFromValidityState(): { code: string; message: React.ReactNode } | null {
    ...
    /** Length of the timestamp entity in the current block, including the trailing space */
    ...
    /** Updates editorState and rechecks validity */
    handleChange = (nextEditorState: EditorState): void => {
        const { internalEditorState, isTimestampToggledOn }: DraftJSMentionSelectorState = this.state;
        const { onChange }: DraftJSMentionSelectorProps = this.props;
    ...
    handleValidityStateUpdateHandler = (): void => {
    ...
    getVideoTimestamp = (): VideoTimestamp => {
        const videoContainer: HTMLElement | null | undefined = document.querySelector('.bp-media-container');
        const video: HTMLVideoElement | null | undefined = videoContainer?.querySelector('video');
