...
export interface Mention {
    /** DraftJS content-block key that contains the mention */
    blockID: string;
    /** Cursor offset where the in-progress mention ends */
    end: number;
    /** Text typed after the mention trigger */
    mentionString: string;
    /** Character that started the mention */
    mentionTrigger: string;
    /** Offset where the in-progress mention starts */
    start: number;
}

interface MentionEntity {
    /** Identifier stored on the inserted mention entity */
    id: string | number;
    /** Display name inserted after the mention trigger */
    name: string;
}
...
const getActiveMentionForEditorState = (
...
const addMention = (editorState: EditorState, activeMention: Mention | null, mention: MentionEntity): EditorState => {
    ...
    return EditorState.push(editorState, contentStateWithLinkAndExtraSpace, 'change-block-type');
};
...
const getFormattedCommentText = (editorState: EditorState): { hasMention: boolean; text: string } => {
    const contentState = editorState.getCurrentContent();
