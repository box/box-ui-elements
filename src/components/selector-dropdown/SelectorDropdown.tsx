...
const stopDefaultEvent = (event: React.SyntheticEvent | Event) => {
...
interface SelectorDropdownState {
    /** ID of the currently active dropdown item for aria-activedescendant */
    activeItemID: string | null;
    /** Index of the currently active dropdown item, or -1 when none is active */
    activeItemIndex: number;
    /** Whether the dropdown should be open based on user interaction */
    shouldOpen: boolean;
}

class SelectorDropdown extends React.Component<SelectorDropdownProps, SelectorDropdownState> {
    static defaultProps = {
        isPositionDynamic: false,
    };

    constructor(props: SelectorDropdownProps) {
        super(props);
    ...
    componentDidUpdate({ shouldSetActiveItemOnOpen, children }: SelectorDropdownProps) {
    ...
    selectorDropdownRef: React.RefObject<HTMLDivElement | null>;

    haveChildrenChanged = (prevChildren?: React.ReactNode) => {
    ...
    selectItem = (index: number, event: React.SyntheticEvent) => {
    ...
    render() {
        ...
        const { dividerIndex, overlayTitle, children, className, isPositionDynamic, title, selector, shouldScroll } =
            this.props;
                ...
                {React.Children.map(children, (item, index) => {
                    ...
                    return (
                        <>
                            ...
                            {React.cloneElement(item as React.ReactElement, itemProps)}
                        </>
                    );
                })}
            </ul>
