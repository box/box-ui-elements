...
class SidebarSkills extends React.PureComponent<Props> {
    componentDidMount() {
        ...
        viewer.addListener('load', () => {
            viewer.loadAutoGeneratedCaptions(cards);
        });
    }

    componentWillReceiveProps(nextProps: Props) {
        const { file, getPreviewer }: Props = nextProps;
        const { cards }: SkillCards = getProp(file, 'metadata.global.boxSkillsCards', []);
        ...
        if (
            ...
            viewer.isLoaded() &&
            !viewer.isDestroyed() &&
            typeof viewer.editAutoGeneratedCaptions === 'function'
        ) {
            viewer.editAutoGeneratedCaptions(cards);
        }
    }
     ...
     */
    getCardInteractionTarget = ({ skill_card_type }: SkillCard): string => {
     ...
     */
    getCardTitle = ({ skill_card_type, skill_card_title = {} }: SkillCard): string | React.Node => {
     ...
     */
    render() {
        const { file, getPreviewer, onSkillChange }: Props = this.props;
        const { cards } = getProp(file, 'metadata.global.boxSkillsCards', []);
        ...
        return cards.map((card: SkillCard, index: number) => {
            ...
            const interactionTarget = this.getCardInteractionTarget(card);
