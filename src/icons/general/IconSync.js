...
const IconSync = ({ className = '', color = '#979797', height = 16, title, width = 16 }: Props) => (
    <AccessibleSVG className={`icon-sync ${className}`} title={title} width={width} height={height} viewBox="0 0 16 16">
        <g className="stroke-color" transform="translate(1 1)" stroke={color} fill="none" fillRule="evenodd">
            <circle cx="7" cy="7" r="7" />
            <path strokeLinecap="round" strokeLinejoin="round" d="M4 7.054l2.58 2.69L10.938 5" />
        </g>
    </AccessibleSVG>
);
