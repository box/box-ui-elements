...
const getCrypto = (): Crypto => window.crypto || (window as WindowWithMsCrypto).msCrypto;

/** Returns a digest promise; uses the msCrypto callback API when native SubtleCrypto is unavailable. */
const digest = (algorithm: string, buffer: ArrayBuffer): Promise<ArrayBuffer> => {
