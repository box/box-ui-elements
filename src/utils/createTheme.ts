...
function getYiq(color: string): number {
    const rgb = Color(color).rgb().array();

    return (rgb[0] * 2126 + rgb[1] * 7152 + rgb[2] * 722) / 10000;
}

function adjustLightness(color: ReturnType<typeof Color>, amount: number): ReturnType<typeof Color> {
...
function createTheme(colorKey: string) {
    ...
    const modifiedColors: Record<string, ReturnType<typeof Color>> = {};
