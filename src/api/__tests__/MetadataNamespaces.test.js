...
describe('api/MetadataNamespaces', () => {
    ...
    describe('getMetadataNamespaceMode()', () => {
        ...
        test('should use the host metadata token when fetching enterprise_configurations', async () => {
            getMetadataAuthToken.mockResolvedValue('meta-token');
            host.xhr.get.mockImplementation(() => {
                expect(host.xhr.token).toBe('meta-token');
                return Promise.resolve(configResponse);
            });

            await expect(api.getMetadataNamespaceMode(file, '123')).resolves.toBe(METADATA_SCOPE_MODE_MIGRATION);

            expect(getMetadataAuthToken).toHaveBeenCalledTimes(1);
            expect(host.xhr.get).toHaveBeenCalled();
            expect(host.xhr.token).toBe('preview-token');
        });

        test('should fetch enterprise_configurations from api.box.com when there is no host token getter', async () => {
            host = createHost({ getMetadataAuthToken: undefined });
            api = new MetadataNamespaces(host);
            host.xhr.get.mockResolvedValue(configResponse);

            await expect(api.getMetadataNamespaceMode(file, '123')).resolves.toBe(METADATA_SCOPE_MODE_MIGRATION);

            expect(host.xhr.get).toHaveBeenCalledWith({
                id: 'file_123',
                url: 'https://api.box.com/2.0/enterprise_configurations/123',
                params: { categories: 'content_and_sharing' },
                headers: { 'box-version': '2025.0' },
            });
        });

        test('should map REST configFlags to MIGRATION', async () => {
        ...
        test('should map REST configFlags to FINAL', async () => {
            host.xhr.get.mockResolvedValue({
                data: {
                    content_and_sharing: {
            ...
            await expect(api.getMetadataNamespaceMode(file, '123')).resolves.toBe(METADATA_SCOPE_MODE_FINAL);
        });
    });
