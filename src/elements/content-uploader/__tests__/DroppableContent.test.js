
var __assign =
    (this && this.__assign) ||
    function () {
        __assign =
            Object.assign ||
            function (t) {
                for (var s, i = 1, n = arguments.length; i < n; i++) {
                    s = arguments[i];
                    for (const p in s) if (Object.prototype.hasOwnProperty.call(s, p)) t[p] = s[p];
                }
                return t;
            };
        return __assign.apply(this, arguments);
    };
const __awaiter =
    (this && this.__awaiter) ||
    function (thisArg, _arguments, P, generator) {
        function adopt(value) {
            return value instanceof P
                ? value
                : new P((resolve) => {
                      resolve(value);
                  });
        }
        return new (P || (P = Promise))((resolve, reject) => {
            function fulfilled(value) {
                try {
                    step(generator.next(value));
                } catch (e) {
                    reject(e);
                }
            }
            function rejected(value) {
                try {
                    step(generator.throw(value));
                } catch (e) {
                    reject(e);
                }
            }
            function step(result) {
                result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected);
            }
            step((generator = generator.apply(thisArg, _arguments || [])).next());
        });
    };
const __generator =
    (this && this.__generator) ||
    function (thisArg, body) {
        let _ = {
                label: 0,
                sent () {
                    if (t[0] & 1) throw t[1];
                    return t[1];
                },
                trys: [],
                ops: [],
            };
            let f;
            let y;
            let t;
            let g;
        return (
            (g = { next: verb(0), throw: verb(1), return: verb(2) }),
            typeof Symbol === 'function' &&
                (g[Symbol.iterator] = function () {
                    return this;
                }),
            g
        );
        function verb(n) {
            return function (v) {
                return step([n, v]);
            };
        }
        function step(op) {
            if (f) throw new TypeError('Generator is already executing.');
            while ((g && ((g = 0), op[0] && (_ = 0)), _))
                try {
                    if (
                        ((f = 1),
                        y &&
                            (t =
                                op[0] & 2
                                    ? y.return
                                    : op[0]
                                      ? y.throw || ((t = y.return) && t.call(y), 0)
                                      : y.next) &&
                            !(t = t.call(y, op[1])).done)
                    )
                        return t;
                    if (((y = 0), t)) op = [op[0] & 2, t.value];
                    switch (op[0]) {
                        case 0:
                        case 1:
                            t = op;
                            break;
                        case 4:
                            _.label++;
                            return { value: op[1], done: false };
                        case 5:
                            _.label++;
                            y = op[1];
                            op = [0];
                            continue;
                        case 7:
                            op = _.ops.pop();
                            _.trys.pop();
                            continue;
                        default:
                            if (
                                !((t = _.trys), (t = t.length > 0 && t[t.length - 1])) &&
                                (op[0] === 6 || op[0] === 2)
                            ) {
                                _ = 0;
                                continue;
                            }
                            if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) {
                                _.label = op[1];
                                break;
                            }
                            if (op[0] === 6 && _.label < t[1]) {
                                _.label = t[1];
                                t = op;
                                break;
                            }
                            if (t && _.label < t[2]) {
                                _.label = t[2];
                                _.ops.push(op);
                                break;
                            }
                            if (t[2]) _.ops.pop();
                            _.trys.pop();
                            continue;
                    }
                    op = body.call(thisArg, _);
                } catch (e) {
                    op = [6, e];
                    y = 0;
                } finally {
                    f = t = 0;
                }
            if (op[0] & 5) throw op[1];
            return { value: op[0] ? op[1] : void 0, done: true };
        }
    };
Object.defineProperty(exports, '__esModule', { value: true });
const React = require('react');
const testing_library_1 = require('../../../test-utils/testing-library');
const DroppableContent_1 = require('../DroppableContent');

describe('elements/content-uploader/DroppableContent', () => {
    beforeEach(() => {
        jest.clearAllMocks();
    });
    const defaultProps = {
        addDataTransferItemsToUploadQueue: jest.fn(),
        addFiles: jest.fn(),
        allowedTypes: ['Files', 'image/png'],
        canDrop: true,
        isFolderUploadEnabled: true,
        isOver: false,
        isTouch: false,
        items: [],
        onClick: jest.fn(),
        view: 'grid',
    };
    const renderComponent = function (props) {
        return (0, testing_library_1.render)(<DroppableContent_1.default {...defaultProps} {...props} />);
    };
    test('addDataTransferItemsToUploadQueue function is called after valid file is dropped', () => {
        return __awaiter(void 0, void 0, void 0, function () {
            let mockAddDataTransferItemsToUploadQueue;
                let testProps;
                let droppableElement;
                let file;
                let mockDataTransfer;
                let mockEvent;
                let actualCall;
            return __generator(this, (_a) => {
                switch (_a.label) {
                    case 0:
                        mockAddDataTransferItemsToUploadQueue = jest.fn();
                        testProps = {...defaultProps, addDataTransferItemsToUploadQueue: mockAddDataTransferItemsToUploadQueue,
                            allowedTypes: ['Files'],
                            canDrop: true,
                            isFolderUploadEnabled: true,};
                        renderComponent(testProps);
                        droppableElement = testing_library_1.screen.getByTestId('bcu-droppable-content');
                        expect(droppableElement).toBeInTheDocument();
                        file = new File(['123'], 'example.png', { type: 'image/png' });
                        mockDataTransfer = {
                            files: [file],
                            items: [
                                {
                                    kind: 'file',
                                    type: 'image/png',
                                    getAsFile () {
                                        return file;
                                    },
                                },
                            ],
                            types: ['Files'],
                            effectAllowed: 'all',
                            dropEffect: 'copy',
                            setData: jest.fn(),
                            getData: jest.fn(),
                            clearData: jest.fn(),
                        };
                        mockEvent = {
                            dataTransfer: mockDataTransfer,
                            preventDefault: jest.fn(),
                            stopPropagation: jest.fn(),
                        };
                        // Simulate complete drag and drop sequence
                        return [
                            4 /* yield */,
                            (0, testing_library_1.act)(() => {
                                return __awaiter(void 0, void 0, void 0, function () {
                                    return __generator(this, (_a) => {
                                        switch (_a.label) {
                                            case 0:
                                                // First trigger dragenter
                                                testing_library_1.fireEvent.dragEnter(droppableElement, mockEvent);
                                                return [
                                                    4 /* yield */,
                                                    new Promise((resolve) => {
                                                        return setTimeout(resolve, 50);
                                                    }),
                                                ];
                                            case 1:
                                                _a.sent();
                                                // Then dragover to maintain drag state
                                                testing_library_1.fireEvent.dragOver(droppableElement, mockEvent);
                                                return [
                                                    4 /* yield */,
                                                    new Promise((resolve) => {
                                                        return setTimeout(resolve, 50);
                                                    }),
                                                ];
                                            case 2:
                                                _a.sent();
                                                // Finally drop
                                                testing_library_1.fireEvent.drop(droppableElement, mockEvent);
                                                return [
                                                    4 /* yield */,
                                                    new Promise((resolve) => {
                                                        return setTimeout(resolve, 50);
                                                    }),
                                                ];
                                            case 3:
                                                _a.sent();
                                                return [2 /* return */];
                                        }
                                    });
                                });
                            }),
                        ];
                    case 1:
                        // Simulate complete drag and drop sequence
                        _a.sent();
                        expect(mockAddDataTransferItemsToUploadQueue).toHaveBeenCalled();
                        actualCall = mockAddDataTransferItemsToUploadQueue.mock.calls[0][0];
                        expect(actualCall.files).toEqual(mockDataTransfer.files);
                        expect(actualCall.items).toEqual(mockDataTransfer.items);
                        expect(actualCall.types).toEqual(mockDataTransfer.types);
                        return [2 /* return */];
                }
            });
        });
    });
    test('addDataTransferItemsToUploadQueue function is not called after invalid file is dropped', () => {
        return __awaiter(void 0, void 0, void 0, function () {
            let mockAddDataTransferItemsToUploadQueue; let droppableElement; let mockDataTransfer;
            return __generator(this, (_a) => {
                switch (_a.label) {
                    case 0:
                        mockAddDataTransferItemsToUploadQueue = jest.fn();
                        renderComponent({
                            addDataTransferItemsToUploadQueue: mockAddDataTransferItemsToUploadQueue,
                            allowedTypes: ['jpg'],
                            canDrop: true,
                        });
                        droppableElement = testing_library_1.screen.getByTestId('bcu-droppable-content');
                        expect(droppableElement).toBeInTheDocument();
                        mockDataTransfer = {
                            files: [new File(['123'], 'example.png', { type: 'image/png' })],
                            items: [
                                {
                                    kind: 'file',
                                    type: 'image/png',
                                    getAsFile () {
                                        return new File(['123'], 'example.png', { type: 'image/png' });
                                    },
                                },
                            ],
                            types: ['Files', 'image/png'],
                            effectAllowed: 'all',
                            dropEffect: 'copy',
                            setData: jest.fn(),
                            getData: jest.fn(),
                            clearData: jest.fn(),
                            preventDefault: jest.fn(),
                            stopPropagation: jest.fn(),
                        };
                        return [
                            4 /* yield */,
                            (0, testing_library_1.act)(() => {
                                return __awaiter(void 0, void 0, void 0, function () {
                                    return __generator(this, (_a) => {
                                        switch (_a.label) {
                                            case 0:
                                                testing_library_1.fireEvent.dragEnter(droppableElement, {
                                                    dataTransfer: mockDataTransfer,
                                                    preventDefault: jest.fn(),
                                                    stopPropagation: jest.fn(),
                                                });
                                                return [
                                                    4 /* yield */,
                                                    new Promise((resolve) => {
                                                        return setTimeout(resolve, 50);
                                                    }),
                                                ];
                                            case 1:
                                                _a.sent();
                                                return [2 /* return */];
                                        }
                                    });
                                });
                            }),
                        ];
                    case 1:
                        _a.sent();
                        return [
                            4 /* yield */,
                            (0, testing_library_1.act)(() => {
                                return __awaiter(void 0, void 0, void 0, function () {
                                    return __generator(this, (_a) => {
                                        switch (_a.label) {
                                            case 0:
                                                testing_library_1.fireEvent.dragOver(droppableElement, {
                                                    dataTransfer: mockDataTransfer,
                                                    preventDefault: jest.fn(),
                                                    stopPropagation: jest.fn(),
                                                });
                                                return [
                                                    4 /* yield */,
                                                    new Promise((resolve) => {
                                                        return setTimeout(resolve, 50);
                                                    }),
                                                ];
                                            case 1:
                                                _a.sent();
                                                return [2 /* return */];
                                        }
                                    });
                                });
                            }),
                        ];
                    case 2:
                        _a.sent();
                        return [
                            4 /* yield */,
                            (0, testing_library_1.act)(() => {
                                return __awaiter(void 0, void 0, void 0, function () {
                                    return __generator(this, (_a) => {
                                        switch (_a.label) {
                                            case 0:
                                                testing_library_1.fireEvent.drop(droppableElement, {
                                                    dataTransfer: mockDataTransfer,
                                                    preventDefault: jest.fn(),
                                                    stopPropagation: jest.fn(),
                                                });
                                                return [
                                                    4 /* yield */,
                                                    new Promise((resolve) => {
                                                        return setTimeout(resolve, 50);
                                                    }),
                                                ];
                                            case 1:
                                                _a.sent();
                                                return [2 /* return */];
                                        }
                                    });
                                });
                            }),
                        ];
                    case 3:
                        _a.sent();
                        expect(mockAddDataTransferItemsToUploadQueue).toHaveBeenCalledTimes(0);
                        return [2 /* return */];
                }
            });
        });
    });
    test('addDataTransferItemsToUploadQueue function is called after valid file is dropped and types are DOMStringList', () => {
        return __awaiter(void 0, void 0, void 0, function () {
            let mockAddDataTransferItemsToUploadQueue;
                let testProps;
                let droppableElement;
                let file;
                let mockDOMStringList;
                let mockDataTransfer;
                let actualCall;
            let _a;
            return __generator(this, (_b) => {
                switch (_b.label) {
                    case 0:
                        mockAddDataTransferItemsToUploadQueue = jest.fn();
                        testProps = {
                            addDataTransferItemsToUploadQueue: mockAddDataTransferItemsToUploadQueue,
                            allowedTypes: ['Files'],
                            canDrop: true,
                        };
                        renderComponent(testProps);
                        droppableElement = testing_library_1.screen.getByTestId('bcu-droppable-content');
                        expect(droppableElement).toBeInTheDocument();
                        file = new File(['123'], 'example.png', { type: 'image/png' });
                        mockDOMStringList =
                            ((_a = {
                                contains: jest.fn((strToSearch) => {
                                    return strToSearch === 'Files';
                                }),
                                item: jest.fn((index) => {
                                    return index === 0 ? 'Files' : null;
                                }),
                                length: 1,
                            }),
                            (_a[Symbol.iterator] = function () {
                                return __generator(this, (_a) => {
                                    switch (_a.label) {
                                        case 0:
                                            return [4 /* yield */, 'Files'];
                                        case 1:
                                            _a.sent();
                                            return [2 /* return */];
                                    }
                                });
                            }),
                            _a);
                        mockDataTransfer = {
                            files: [file],
                            items: [
                                {
                                    kind: 'file',
                                    type: 'image/png',
                                    getAsFile () {
                                        return file;
                                    },
                                },
                            ],
                            types: mockDOMStringList,
                            effectAllowed: 'all',
                            dropEffect: 'copy',
                            setData: jest.fn(),
                            getData: jest.fn(),
                            clearData: jest.fn(),
                            preventDefault: jest.fn(),
                            stopPropagation: jest.fn(),
                        };
                        // Simulate complete drag and drop sequence
                        return [
                            4 /* yield */,
                            (0, testing_library_1.act)(() => {
                                return __awaiter(void 0, void 0, void 0, function () {
                                    return __generator(this, (_a) => {
                                        switch (_a.label) {
                                            case 0:
                                                // First trigger dragenter
                                                testing_library_1.fireEvent.dragEnter(droppableElement, {
                                                    dataTransfer: mockDataTransfer,
                                                    preventDefault: jest.fn(),
                                                    stopPropagation: jest.fn(),
                                                });
                                                return [
                                                    4 /* yield */,
                                                    new Promise((resolve) => {
                                                        return setTimeout(resolve, 50);
                                                    }),
                                                ];
                                            case 1:
                                                _a.sent();
                                                // Then dragover
                                                testing_library_1.fireEvent.dragOver(droppableElement, {
                                                    dataTransfer: mockDataTransfer,
                                                    preventDefault: jest.fn(),
                                                    stopPropagation: jest.fn(),
                                                });
                                                return [
                                                    4 /* yield */,
                                                    new Promise((resolve) => {
                                                        return setTimeout(resolve, 50);
                                                    }),
                                                ];
                                            case 2:
                                                _a.sent();
                                                // Finally drop
                                                testing_library_1.fireEvent.drop(droppableElement, {
                                                    dataTransfer: mockDataTransfer,
                                                    preventDefault: jest.fn(),
                                                    stopPropagation: jest.fn(),
                                                });
                                                return [
                                                    4 /* yield */,
                                                    new Promise((resolve) => {
                                                        return setTimeout(resolve, 50);
                                                    }),
                                                ];
                                            case 3:
                                                _a.sent();
                                                return [2 /* return */];
                                        }
                                    });
                                });
                            }),
                        ];
                    case 1:
                        // Simulate complete drag and drop sequence
                        _b.sent();
                        expect(mockAddDataTransferItemsToUploadQueue).toHaveBeenCalled();
                        actualCall = mockAddDataTransferItemsToUploadQueue.mock.calls[0][0];
                        expect(actualCall.files).toEqual(mockDataTransfer.files);
                        expect(actualCall.items).toEqual(mockDataTransfer.items);
                        expect(actualCall.types).toBe(mockDataTransfer.types);
                        return [2 /* return */];
                }
            });
        });
    });
    test('addDataTransferItemsToUploadQueue function is not called after invalid file is dropped and types are DOMStringList', () => {
        let _a;
        const mockAddDataTransferItemsToUploadQueue = jest.fn();
        renderComponent({
            addDataTransferItemsToUploadQueue: mockAddDataTransferItemsToUploadQueue,
            allowedTypes: ['jpg'],
            canDrop: true,
        });
        const droppableElement = testing_library_1.screen.getByTestId('bcu-droppable-content');
        expect(droppableElement).toBeInTheDocument();
        const mockDOMStringList =
            ((_a = {
                contains: jest.fn((strToSearch) => {
                    return strToSearch === 'png';
                }),
                item: jest.fn((index) => {
                    return index === 0 ? 'png' : null;
                }),
                length: 1,
            }),
            (_a[Symbol.iterator] = function () {
                return __generator(this, (_a) => {
                    switch (_a.label) {
                        case 0:
                            return [4 /* yield */, 'png'];
                        case 1:
                            _a.sent();
                            return [2 /* return */];
                    }
                });
            }),
            _a);
        const mockDataTransfer = {
            files: [new File(['123'], 'example.png')],
            types: mockDOMStringList,
            effectAllowed: 'none',
            dropEffect: 'none',
            setData: jest.fn(),
            getData: jest.fn(),
            clearData: jest.fn(),
        };
        testing_library_1.fireEvent.dragEnter(droppableElement, { dataTransfer: mockDataTransfer });
        testing_library_1.fireEvent.dragOver(droppableElement, { dataTransfer: mockDataTransfer });
        testing_library_1.fireEvent.drop(droppableElement, { dataTransfer: mockDataTransfer });
        expect(mockAddDataTransferItemsToUploadQueue).toHaveBeenCalledTimes(0);
    });
});
