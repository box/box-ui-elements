...
describe('elements/common/annotator-context/withAnnotations', () => {
    ...
    describe('handleActiveChange()', () => {
        ...
        test('should call onAnnotationStateChange with annotationId and isPopupOpen', () => {
            const onAnnotationStateChange = jest.fn();
            const wrapper = getWrapper({ onAnnotationStateChange });

            mockAnnotator.isPopupOpen = jest.fn().mockReturnValue(true);
            wrapper.instance().handleAnnotator(mockAnnotator);
            wrapper.instance().handleActiveChange({ annotationId: '123', fileVersionId: '456' });
        ...
        test('should default isPopupOpen to false if annotator does not have isPopupOpen', () => {
            const onAnnotationStateChange = jest.fn();
            const wrapper = getWrapper({ onAnnotationStateChange });
        ...
        test('should still update state when onAnnotationStateChange is not provided', () => {
            const wrapper = getWrapper();
            ...
            const contextProvider = getContextProvider(wrapper);
            expect(contextProvider.prop('value').state.activeAnnotationId).toBe('123');
            expect(contextProvider.prop('value').state.activeAnnotationFileVersionId).toBe('456');
        });

        test('should still update state if onAnnotationStateChange throws', () => {
            const onAnnotationStateChange = jest.fn().mockImplementation(() => {
                throw new Error('callback error');
            });
            const onError = jest.fn();
            const wrapper = getWrapper({ onAnnotationStateChange, onError });
            ...
            const contextProvider = getContextProvider(wrapper);
            expect(contextProvider.prop('value').state.activeAnnotationId).toBe('123');
            expect(onError).toHaveBeenCalledWith(expect.any(Error), 'annotation_state_change_callback_error', {
    ...
    describe('handleAnnotator()', () => {
        test('should call onAnnotatorReady with control methods', () => {
            const onAnnotatorReady = jest.fn();
            const wrapper = getWrapper({ onAnnotatorReady });

            mockAnnotator.getActiveId = jest.fn().mockReturnValue('abc');
            mockAnnotator.getAnnotationContainerEl = jest.fn().mockReturnValue(document.createElement('div'));
            mockAnnotator.isPopupOpen = jest.fn().mockReturnValue(true);

            wrapper.instance().handleAnnotator(mockAnnotator);

            expect(onAnnotatorReady).toHaveBeenCalledTimes(1);
            const controls = onAnnotatorReady.mock.calls[0][0];
            expect(controls).toHaveProperty('deselect');
            expect(controls).toHaveProperty('getActiveId');
            expect(controls).toHaveProperty('getAnnotationContainerEl');
            expect(controls).toHaveProperty('isPopupOpen');
        });

        test('should provide a deselect control that emits null active annotation', () => {
            const onAnnotatorReady = jest.fn();
            const wrapper = getWrapper({ onAnnotatorReady });
            ...
            expect(mockAnnotator.emit).toHaveBeenCalledWith('annotations_active_set', null);
        });

        test('should provide a getActiveId control that delegates to annotator', () => {
            const onAnnotatorReady = jest.fn();
            const wrapper = getWrapper({ onAnnotatorReady });
            ...
            const controls = onAnnotatorReady.mock.calls[0][0];
            expect(controls.getActiveId()).toBe('abc');
        });

        test('should provide a getActiveId control that returns null if annotator lacks getActiveId', () => {
            const onAnnotatorReady = jest.fn();
            const wrapper = getWrapper({ onAnnotatorReady });
            ...
            const controls = onAnnotatorReady.mock.calls[0][0];
            expect(controls.getActiveId()).toBeNull();
        });

        test('should provide an isPopupOpen control that delegates to annotator', () => {
            const onAnnotatorReady = jest.fn();
            const wrapper = getWrapper({ onAnnotatorReady });
        ...
        test('should provide an isPopupOpen control that returns false if annotator lacks isPopupOpen', () => {
            const onAnnotatorReady = jest.fn();
            const wrapper = getWrapper({ onAnnotatorReady });
        ...
        test('should still register event listeners when onAnnotatorReady is not provided', () => {
            const wrapper = getWrapper();

            wrapper.instance().handleAnnotator(mockAnnotator);

            expect(mockAnnotator.addListener).toHaveBeenCalledWith(
        ...
        test('should still register event listeners if onAnnotatorReady throws', () => {
            const onAnnotatorReady = jest.fn().mockImplementation(() => {
                throw new Error('callback error');
            });
            const onError = jest.fn();
            const wrapper = getWrapper({ onAnnotatorReady, onError });
            ...
            expect(onError).toHaveBeenCalledWith(expect.any(Error), 'annotator_ready_callback_error', {
                showNotification: false,
            });
        });
    });
    ...
    describe('handlePreviewDestroy()', () => {
        ...
        test('should call onAnnotatorReady with null', () => {
            const onAnnotatorReady = jest.fn();
            const wrapper = getWrapper({ onAnnotatorReady });

            wrapper.instance().handleAnnotator(mockAnnotator);
            onAnnotatorReady.mockClear();
            wrapper.instance().handlePreviewDestroy();

            expect(onAnnotatorReady).toHaveBeenCalledWith(null);
        });

        test('should call onAnnotationStateChange with null activeAnnotationId and false isPopupOpen', () => {
            const onAnnotationStateChange = jest.fn();
            const wrapper = getWrapper({ onAnnotationStateChange });
        ...
        test('should call onAnnotatorReady with null and onAnnotationStateChange even when shouldReset is false', () => {
            const onAnnotatorReady = jest.fn();
            const onAnnotationStateChange = jest.fn();
            const wrapper = getWrapper({ onAnnotationStateChange, onAnnotatorReady });
        ...
        test('should still call onAnnotationStateChange if onAnnotatorReady throws', () => {
            const onAnnotatorReady = jest.fn();
            const onAnnotationStateChange = jest.fn();
            const onError = jest.fn();
            const wrapper = getWrapper({ onAnnotationStateChange, onAnnotatorReady, onError });

            wrapper.instance().handleAnnotator(mockAnnotator);
            onAnnotatorReady.mockClear();
            onAnnotatorReady.mockImplementation(() => {
                throw new Error('callback error');
            });
            wrapper.instance().handlePreviewDestroy();
