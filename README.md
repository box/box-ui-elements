...
# Code Splitting
[Code splitting](https://webpack.js.org/guides/code-splitting/) can be enabled for certain elements. As of now, the elements which support code splitting are `ContentPreview` and `ContentSidebar`. This is done using [react-loadable](https://github.com/jamiebuilds/react-loadable), but in a future release will change to `React.lazy`. In order to use code splitting, you will need to build an element with webpack by importing the element from the `lib` folder in our npm package.
